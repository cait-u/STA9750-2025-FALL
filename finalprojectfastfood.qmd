...
According to the [CDC](https://blogs.cdc.gov/nchs/2025/06/26/7784/), 32% of adults in the United States consumed fast food on any given day between August 2021 and August 2023, making access to fast food an important factor when examining potential health-related risks. This project analyzes the geographic distribution of fast food restaurants across the United States to answer the question: **How many fast food restaurants per capita are there in each state and county?**

The motivation for this analysis is rooted in the overarching question: **Are food deserts in the US linked to health problems like diabetes?** Understanding where fast food is most concentrated provides insight into differences in food access across communities and helps identify regions that have less quality or healthy food options. For further information about this project and to see other analytical questions tied to this investigation, click [here](https://festrada0817.github.io/STA9750-2025-FALL/FinalProjectPresentation2025.html).
...
The [U.S. Census Bureau’s American Community Survey (ACS) Table B01003](https://data.census.gov/table/ACSDT5Y2019.B01003?q=Population+Total&g=010XX00US$1400000) provides estimates of total population for a given geographic area, serving as a foundational demographic measure in this analysis. The ACS B01003 table used here is based on the 2019 five year estimates, which are designed to provide more reliable population figures, particularly for smaller geographic units such as census tracts and counties. The ACS dataset was downloaded manually and created a reproducible import pipeline that verifies file availability and performs all subsequent processing steps.
...
pop_2019<- read_csv(pop_file, show_col_types = FALSE)
```
...
Looking at summary statistics helps understand the distribution of fast food availability across census tract.

#### Fast Food Count

The distribution of fast food restaurants across census tracts is highly right-skewed, as a large share of census tracts have no fast food restaurants at all. As the number of restaurants increases, the number of census tracts declines rapidly, indicating that high fast-food counts are concentrated in a relatively small number of locations. This pattern is reflected in the summary statistics, where the minimum, first quartile, and median are all zero, while the mean is higher at 0.815 due to a small number of tracts with many restaurants. The maximum value of 22 highlights the presence of extreme outliers that pull the average upward, suggesting substantial variation in fast food availability across communities.
...
#### Total Population
The distribution of population across census tracts is relatively contained and mildly right-skewed, with most tracts falling between approximately 2,000 and 6,000 residents and very few extreme outliers. The median population is 3,852, which represents the size of a typical census tract and aligns with Census Bureau guidelines for tract population ranges. This relatively tight distribution suggests that large differences in fast food density across tracts are less likely to be driven solely by population size and more likely reflect differences in the number of fast food restaurants present.
```{r}
...
This choropleth shows a clear regional pattern, particularly in the South, where darker shading indicates a higher density of fast food restaurants compared to the Northeast, Midwest, and much of the West. A notable exception appears in the West, where one state exhibits relatively high fast food density despite the broader regional trend. To better understand these patterns, we next examine a datatable displaying state-level fast food density alongside the corresponding total number of fast food restaurants and total population.popultation.
```{r}
# Create heat map 
...
This pattern suggests that many Southern states, along with select western regions, may face greater challenges related to food access, where communities are more heavily saturated with convenient fast food options compared to healthier alternatives.
```{r}
# Create datatable
datatable(
  fastfood_map_state |>
...
Similar to the state level analysis, let’s take a closer look at the choropleth map at the county level to identify more localized patterns in fast food density. To do this, we aggregate census tract level fast food counts and population data up to the county level. We then load and join a U.S. county shapefile to visualize how fast food density varies across counties and to highlight regional clustering.
```{r}
#State look up
